...
```bash
ssh -o IPQoS=none root@143.95.219.248 -p 22022 "cd /root/aplicacao && git pull && systemctl restart streamlit"
```

## 3. Principais Ações e Seus Comandos Remotos
- **Atualizar Aplicação:** `cd /root/aplicacao && git pull && systemctl restart streamlit`
...
- **Ver Status do Serviço:** `systemctl status streamlit`
